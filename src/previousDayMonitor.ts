...
export async function runPreviousDayMonitor() {
  try {
    ...
    let alertMessage =
      'Something bad happened. There seems to be an issue with monitoring HFP-data. Investigate and fix the problem.'
...
async function previousDayMonitor() {
  ...
  console.log(
    `Running HFP sink previous day monitor for container: ${HFP_STORAGE_CONTAINER_NAME}`
  )
