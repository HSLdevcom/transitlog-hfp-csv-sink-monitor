...
import {
  HFP_MONITOR_PULSAR_BOOKIE_IP_1,
  HFP_MONITOR_PULSAR_BOOKIE_IP_2,
  HFP_MONITOR_PULSAR_BOOKIE_IP_3,
  HFP_MONITOR_PULSAR_BOOKIE_DISK_SPACE_PORT,
} from './constants'
...
export async function runAvailableDiskSpaceMonitor() {
  try {
    ...
    let alertMessage =
      'Something bad happened. There seems to be an issue with available disk space monitor. Investigate and fix the problem.'
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('data', async (data) => {
        if (data) {
          ...
          console.log(
            `Available disk space monitoring complete, available disk space of bookie ${bookieIP} was: ${availableDiskSpace}%`
          )
        } else {
