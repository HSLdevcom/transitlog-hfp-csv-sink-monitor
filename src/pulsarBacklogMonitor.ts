...
import {
  HFP_MONITOR_PULSAR_PROXY_IP,
  HFP_MONITOR_PULSAR_ADMIN_PORT,
...
export async function runPulsarBacklogMonitor() {
  try {
    ...
    let alertMessage =
      'Something bad happened. There seems to be an issue with pulsar backlog message count. Investigate and fix the problem.'
...
async function pulsarBacklogMonitor() {
  ...
  if (!hfpTopicStats) {
    throw new Error(
      'Could not find hfp topic stats. Has the pulsar IP / tenant / namespace / topic changed recently?'
    )
  }
  ...
  console.log(
    'Pulsar monitoring complete, backlog message count was: ',
    backlogMessageCountInMillions
  )
}
